...
The environment simulates a vulnerable honeypot by deploying a Windows Virtual Machine exposed to the internet to attract real-world attack attempts. Security events generated by the VM are collected and sent to a centralized Log Analytics Workspace, where they are analyzed using Microsoft Sentinel.

The lab includes:
- Deployment of an Azure Virtual Machine configured as a honeypot  
- Configuration of a Log Analytics Workspace for centralized log storage  
- Integration with Microsoft Sentinel for security monitoring and analysis  
- Collection of Windows Security Events using data collection rules  
- Detection of failed login attempts and suspicious activity using KQL  
- Log enrichment with GeoIP data to map IP addresses to geographic locations  
- Creation of an attack map to visualize the origin of attack attempts  

This project demonstrates real-time threat detection, log enrichment, and visualization techniques commonly used in Security Operations Centers (SOC).
